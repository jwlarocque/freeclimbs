<script lang="ts">
	...
	import AddHoldIcon from '$lib/icons/AddHoldIcon.svelte';
...
<svelte:head>
	<title>Freeclimbs | Edit Set</title>
</svelte:head>
<main class="sign">
		...
		{#if showInfo}
			{#if false && editorState == 'init'}
				<ul>
					...
					<li>
						The model may have trouble distinguishing holds which are the same color as the
						background.
					</li>
					<li>
						If you're not happy with the results, you can refresh the page and try again with a new
						image.
					</li>
				</ul>
			{:else if false && editorState == 'processing'}
				<p>This may take a few minutes, particularly if your wall has a lot of holds.</p>
				<p>Using a more powerful computer will speed this up.</p>
			{:else if true || editorState == 'done'}
				<p>
					Review the detected holds and correct any errors. Once you publish the Set you'll no
					longer be able to make any changes, so take your time.
				</p>
				<p>
					Each red box shows the bounds of a detected hold. The blue outlines show their
					automatically detected edges - these are what will be visible to users. Each hold should
					have exactly one box and outline.
				</p>
			{/if}
		{/if}
		<header>
			...
			<button
				...
				on:keypress={() => (showInfo = !showInfo)}
				class="buttonLight">{showInfo ? 'Hide' : 'Show'} tips</button
			>
		</header>
	</div>
</main>
