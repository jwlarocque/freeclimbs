...
<div class="sharing">
				...
				{#each shares as share, i}
					<div class="share">
						<div style="display: flex; flex-direction: row;">
							...
							<CopyButton
								value={`${window.location.origin}/share?secret=${share.id}`}
						...
						<span class="editingButton">
							<BoolInput
								...
								onValueChange={async (editing) => {
									await setShareEditing(share, editing);
									refreshShares();
								}}
							/>
							<p>Allow Setting</p>
						...
						<ConfirmModal buttonClass="buttonDark" title="Users of this link">
							<span slot="button" title="View Users">
								{share?.expand?.users_shares_via_share.length || 0}
								{share?.expand?.users_shares_via_share.length == 1 ? 'user' : 'users'}
							</span>
							<div slot="message">
	...
	<button
		...
		on:click={async () => {
			await createShare(wallId, false);
			refreshShares();
		}}>New Sharing Link</button
	>
</div>
