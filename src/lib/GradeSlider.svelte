<script>
	...
	let system = 'v';
	let unique_grades = grades.filter(
		(grade, index, self) => self.findIndex((g) => g[system] === grade[system]) === index
	);
	let internalValue = [0, unique_grades.length - 1];
	$: if ($userSettingsStore?.grading_system && $userSettingsStore?.grading_system != system) {
		system = $userSettingsStore?.grading_system;
		unique_grades = grades.filter(
			(grade, index, self) => self.findIndex((g) => g[system] === grade[system]) === index
		);
	...
	const debouncedSetValue = debounce((value) => {
		...
		externalValue[0] = grades.findIndex(
			(grade) => grade[system] === unique_grades[value[0]][system]
		);
		// set upper bound to index of last occurence of grade[SYSTEM] in grades
		externalValue[1] = grades.findLastIndex(
			(grade) => grade[system] === unique_grades[value[1]][system]
		);
	}, 200);
