<script lang="ts">
	...
	async function loadRoutes(page = 1) {
		...
		const records = await pb.collection('routes').getList(page, PAGE_SIZE, {
				...
				{#each routesCopy as route (route.id)}
					...
					>
						...
						<div class="ownerActions">
							{#if $authStore.isValid && route.setter == $authStore.model?.id}
								{#if route.draft}
									<button
										class="buttonDarkTwo"
										on:click={() => {
								...
								>
									...
									<button slot="confirm" class="buttonDelete" on:click={() => deleteRoute(route.id)}
										>Delete</button
									>
								</ConfirmModal>
							{/if}
			...
			{#if routesCopy.length > 0}
				...
				>
					...
					{#each pages as page (page.key)}
						...
						{:else}
							<Pagination.Page
								class={page.value == currPage
									? 'buttonDark paginationButton current'
									: 'buttonDark paginationButton'}
								{page}
							/>
						{/if}
					{/each}
